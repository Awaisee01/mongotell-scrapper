    ...
    def scrape(self):
        ...
        for row in rows:
            try:
                ...
                # Col 1: Number (inside .conversation-recent-session-name)
                # We can try to get text of the whole cell first
                number_cell = cols[1]
                number = self.text_content(element_id=number_cell.id).strip()
                
                # Col 3: Message
                message_cell = cols[3]
                message = self.text_content(element_id=message_cell.id).strip()
                
                # Col 4: Time
                time_cell = cols[4]
                time_val = self.text_content(element_id=time_cell.id).strip()
